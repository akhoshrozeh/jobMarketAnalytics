...
export default function RelatedSkillsB() {
    ...
    return (
      <div className="bg-slate-100/50 rounded-lg shadow p-4 shadow-gray-300 shadow-md border border-m-dark-green">
        ...
        <div className="bg-gray-50 rounded-lg p-6 text-center">
          <p className="text-gray-500">Select Skill B to view related skills</p>
        </div>
      </div>
    )
  }

  const relatedSkills = skillB.data?.relatedSkills;
  const marketDemand = skillB.data?.marketDemand;

  if (!relatedSkills || !marketDemand) {
    return (
      <div className="bg-slate-100/50 rounded-lg shadow p-4 shadow-gray-300 shadow-md border border-m-dark-green">
        <h2 className="text-xl font-semibold text-black text-center mb-4">
          🔗 Related Skills {skillB.skill && `- ${skillB.skill}`}
        </h2>
        <div className="bg-gray-50 rounded-lg p-6 text-center">
                ...
                >
                  ...
                  {hoveredIndex === index && rowRefs.current[index] &&
                    createPortal(
                      ...
                      >
                        {skill.percentage}% of jobs that require <strong>{skillB.skill}</strong> also mention <strong>{skill.keyword}</strong>
