...
export default function SkillSalaryStatsB() {
  const { skillB } = useCompareSkills()

  if (!skillB.skill || !skillB.data) {
    return (
      <div className="bg-gray-50 rounded-lg p-6 text-center">
        <p className="text-gray-500">Select Skill B to view salary stats</p>
      </div>
    )
  }

  const stats = skillB.data?.skillSalaryStats;
  const marketSalaryAverage = skillB.data?.marketSalaryAverage?.[0];
  const marketTotalJobs = skillB.data?.marketTotalJobs;
  ...
  return (
    <div className="flex flex-col gap-3 w-full bg-slate-100/50 rounded-lg shadow p-4 shadow-gray-300 shadow-md border border-m-dark-green">
      <h2 className="text-xl font-semibold text-black text-center mb-4">
        📈 Salary Stats {skillB.skill && `- ${skillB.skill}`}
      </h2>
      {/* Average Salary Card */}
